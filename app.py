...
def predict():
    if y == 'weekly':
        ...
        input = pd.DataFrame(scalar_week.transform([x]),
                             columns=['Product_type', 'Promotion_applied', 'Generic_Holiday', 'updated_week',
                                      'updated_month'])

        # predicting
        result = np.round(np.exp(week_model.predict(input)) * 1.1, 0)
        mth = calendar.month_name[int(month)]
        weekMonth = mth + ' week - ' + week
        return render_template('predict.html', res=[product, int(result), weekMonth])
        ...
        mth = calendar.month_name[int(month)]
        return render_template('predict.html', res=[product, int(result), mth])
        ...
        return render_template('predict.html', res=[product, int(result), quartername])
